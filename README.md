...
九个完整实验，把观察、计算、定义与证明接起来。无后端、无登录、无 CDN，可完全离线运行。

[English](README.en.md) · [隐函数指南](docs/IMPLICIT-GUIDE.md) · [导数线性映射指南](docs/LINEAR-GUIDE.md) · [一致收敛指南](docs/UNIFORM-GUIDE.md) · [实际验证](docs/VERIFICATION-v1.8.md) · [交付状态](docs/STATUS.md)
...
**当自由变量改变时，依赖变量如何补偿，才能留在约束面上？**

第九个实验把隐函数定理做成“制造误差 → 一阶修正 → 精确分支”的连续体验：从平面曲线到曲面，再到多约束向量系统；比较自由变量空间、依赖变量空间与约束空间，推导 `Dg = −(D_yF)⁻¹D_xF`，并切换依赖变量、参数化曲线和奇异矩阵案例。

探索、证明、自测三页各有默认展开的「本页导览」，明确告诉第一次进入的学习者本页问题、三步操作、重点观察、结论和常见误区，并链接到完整指南。精确奇异与近奇异、线性修正残差、分支唯一性和公式适用条件都分别处理，避免把图像或数值读数误当成定理证明。详见 [隐函数指南](docs/IMPLICIT-GUIDE.md) 与 [数学推导](docs/IMPLICIT-MATHEMATICS.md)。

## v1.7 · 导数，是局部线性机器

**一个小位移，怎样通过线性映射预测多元函数的输出变化？**

第八个实验用同一映射串起局部线性化、Jacobian 列、换基、链式法则与投影中值定理；比较真实变化、一阶预测和余项，特别区分抽象导数与坐标矩阵、向量等式与方向投影结论。探索、证明、自测三页都配有统一导览。详见 [线性映射指南](docs/LINEAR-GUIDE.md) 与 [数学推导](docs/LINEAR-MATHEMATICS.md)。
...
本地 v1.6 已通过 **275 项 Node 检查**；GitHub Actions Run #23 已通过 **214 项 Chromium 检查**并完成 Pages 部署，详见 [v1.6 验证记录](docs/VERIFICATION-v1.6.md)。新增实验及 v1.5 页面均带有按页面阶段切换的本页导览卡片。

当前环境的浏览器策略阻止 HTTP 导航；本地浏览器检查显式采用 `--offline-harness`，注入**真实构建的完整离线 HTML** 执行，没有修改或规避浏览器策略。HTTP 资源及根路径/仓库子路径另由真实 Node 服务请求检查。远端 Actions Run #23 已通过并完成 Pages 部署，线上地址已做基本验收；仍不声称浏览器 HTTP 模块加载全链路、操作系统文件策略、Safari/Firefox 或完整屏幕阅读器验收。见 [验证记录](docs/VERIFICATION-v1.6.md)。测试不是一般数学证明。
...
`npm run build` 输出纯静态 dist，支持根路径与 `/MA_playground/`。GitHub Actions Run #23 已通过并部署：[线上地址](https://xby474-dev.github.io/MA_playground/)。
