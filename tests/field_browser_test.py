...
try:
    with sync_playwright() as p:
        ...
        expect(direct.locator('#field-scene svg')).to_be_visible();assert direct.locator('.lab-nav').count()==3
        direct.close()
        ok('Direct offline entry opens the unified lab without a hash and retains all three experiments')
