...
try:
    with sync_playwright() as p:
        ...
        expect(page.locator('#field-scene svg')).to_be_visible();assert page.locator('.lab-nav').count()==9  # v1.7 and v1.8 add two labs; existing navigation remains
        ...
        expect(direct.locator('#field-scene svg')).to_be_visible();assert direct.locator('.lab-nav').count()==9
        direct.close()
        ok('Direct offline entry opens the unified lab without a hash and retains all nine experiments')
