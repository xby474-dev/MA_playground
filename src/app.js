...
function render() {
  ...
  main.innerHTML = header(state) + ({ explore, proof, quiz }[state.tab])(state) + footer;
  for (const lab of ['limits', 'differentiability']) {
    const nav = $(`nav-${lab}`);
    if (state.lab === lab) nav.setAttribute('aria-current', 'page'); else nav.removeAttribute('aria-current');
  }
  document.title = `${state.lab === 'limits' ? '所有直线，都不够' : '偏导存在，也不够'} · MA Playground`;
...
document.addEventListener('click',event=>{
  ...
  if(nav){event.preventDefault();const lab=nav.getAttribute('href').includes('differentiability')?'differentiability':'limits';state=defaults(lab);syncURL(true);render();window.scrollTo(0,0);return;}
  if(!button)return;
